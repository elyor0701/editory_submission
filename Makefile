...
copy-proto-module:
	...
	rsync -rv --exclude=.git ${CURRENT_DIR}/editory_protos/* ${CURRENT_DIR}/protos
...
test-migration-up:
	migrate -path ./migrations/postgres -database="postgres://euler:euler@localhost:5432/editory_submission?sslmode=disable&x-migrations-table=migrations_editory_submission" up
	# - migrate -path=$PWD/$PATH_MIGRATION -database="postgres://auth_service:Iegfrte45eatr7ieso@65.109.239.69:5432/auth_service?sslmode=disable&x-migrations-table=migrations_ucode_go_auth_service" up

test-migration-down:
	migrate -path ./migrations/postgres -database 'postgres://euler:euler@localhost:5432/editory_submission?sslmode=disable' down
